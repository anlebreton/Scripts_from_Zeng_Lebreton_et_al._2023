...
# #   facet_wrap(~ Tree_species, scales = "free_x")
# # 
# # h2 <- ggplot(data=df, aes(x = samples, y = 1, fill = Cluster))+
...
# #   facet_wrap(.~Tree_species, scales = "free_x")
# # 
# # h4 <- ggplot(data=df, aes(x = samples, y = 1, fill = Layer))+
# #   geom_bar(stat = "identity", width = 1)+
# #   theme_void()+
# #   theme(panel.spacing.x = unit(1, "mm"))+
# #   facet_wrap(.~Tree_species, scales = "free_x")
...
#   #dplyr::select(-starts_with("NR_"),-starts_with("Myc"),-ID,-ctg_length, -sum_contig_count) %>% 
#   dplyr::select(starts_with(amplicon), Order) %>%
#   group_by(Order) %>% summarise_if(is.numeric, sum) %>% ungroup %>%
#   pivot_longer(cols=-c(Order), names_to="samples", values_to="counts")%>%
#   left_join(., samplesMetaData, by="samples") %>% 
#   group_by(Order) %>%
#   summarise("reads_count"=sum(counts)) %>% ungroup %>%
#   mutate(totalCat=sum(reads_count)) %>% 
#   group_by(Order) %>%  
#   mutate(perc=(sum(reads_count)/totalCat)*100) %>% 
#   arrange(desc(perc)) %>% ungroup %>%
#   mutate(cumul_observation_perc=cumsum(perc)) %>%
#   filter(cumul_observation_perc <= 95) 
# 
# 
# top10Taxo<- data %>%
#   #dplyr::select(-starts_with("NR_"),-starts_with("Myc"),-ID,-ctg_length, -sum_contig_count) %>% 
...
#   pivot_longer(cols=-c(Order), names_to="samples", values_to="counts")%>%
#   left_join(., samplesMetaData, by="samples") %>% 
#   group_by(Order, Tree_species, Cluster) %>%
#   summarise("reads_count"=sum(counts)) %>% ungroup %>%
#   group_by(Tree_species, Cluster) %>%
#   mutate(totalCat=sum(reads_count)) %>% ungroup %>%
#   group_by(Order,Tree_species, Cluster) %>%  
#   mutate(perc=(sum(reads_count)/totalCat)*100) %>% 
#   arrange(desc(perc)) %>% #ungroup %>%
#   mutate(cumul_observation_perc=cumsum(perc)) %>%
...
#   left_join(.,ecology) %>%
#   #dplyr::select(-starts_with("NR_"),-starts_with("Myc"),-ID,-ctg_length, -sum_contig_count) %>% 
#   dplyr::select(starts_with(amplicon), primary_lifestyle) %>%
...
#   group_by(samples,primary_lifestyle, Cluster,Tree_species) %>%
#   summarise("reads_count"=sum(counts)) %>% 
#   ungroup() %>%
...
df <- data %>%
  ...
  left_join(., ecology, by="Genus") %>%
  mutate(primary_lifestyle = case_when(primary_lifestyle %in% top10Taxo$primary_lifestyle ~ primary_lifestyle, TRUE ~ "Others")) %>% 
  group_by(primary_lifestyle, Cluster,Tree_species) %>%
  summarise("reads_count"=sum(counts)) %>% 
  ungroup() %>%
  group_by(Cluster) %>% mutate("reads_percent"= (reads_count/sum(reads_count))*100 ) 
...
pe1 <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=primary_lifestyle))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
samplesMetaData <- read.csv("../0_Metadata/Yulong_sampling_ITS2_metadata.txt", sep='\t')
samplesMetaData <- samplesMetaData %>% mutate(samples= gsub("-", ".", X)) 
data <- read.csv("../1_data/Yulong_ITS2_normalised_abundance_addCols.tsv", sep='\t')
#data[ data == "unidentified" ] <- "Unidentified" 
#data[ data == "Multi-affiliation" ] <- "Unidentified" 
...
pp2 <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=Phylum))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
pc2 <-  ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=Class))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
po2 <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=Order))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
pf2 <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=Family))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
pg2 <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=Genus))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
pe2 <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=primary_lifestyle))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
samplesMetaData <- read.csv("../0_Metadata/Yulong_sampling_AMF_metadata.txt", sep='\t')
samplesMetaData <- samplesMetaData %>% mutate(samples= gsub("-", ".", X)) 
data <- read.csv("../1_data/Yulong_AMF_normalised_abundance_addCols.tsv", sep='\t')
#data[ data == "unidentified" ] <- "Unidentified" 
#data[ data == "Multi-affiliation" ] <- "Unidentified" 
...
ppa <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=Phylum))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
pca <-  ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=Class))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
poa <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=Order))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
pfa <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=Family))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
pga <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=Genus))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
pea <- ggplot(data=df,aes(y=reads_percent,x=Cluster, fill=primary_lifestyle))+
  geom_bar(stat="identity")+
  theme_bw()+
  scale_fill_manual(values=mycolor)+
  theme(axis.text.x=element_text(angle = 55,hjust=1#,size=2
...
(pp1 | pp2 | ppa) / (pc1 | pc2 | pca) / (po1 | po2 | poa) / (pf1 | pf2 | pfa) /(pg1|pg2|pga) / (pe1 | pe2 |pea)
